Shader "NatureManufacture Shaders/Standard Shaders/Standard Metalic UV Free Snow"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult3752 = lerp( tanTriplanarNormal3729 , BlendNormals( tanTriplanarNormal3729 , UnpackScaleNormal( tex2D( _DetailNormalMap, uv_DetailAlbedoMap ) ,_DetailNormalMapScale ) ) , tex2DNode3751.a);
			...
			float3 lerpResult3647 = lerp( BlendNormals( lerpResult3752 , UnpackScaleNormal( tex2D( _ShapeBumpMap, uv_ShapeBumpMap ) ,_ShapeBumpMapScale ) ) , ( tanTriplanarNormal3735 * appendResult3734 ) , ( saturate( ( ase_worldNormal.y * _Snow_Amount ) ) * temp_output_3645_0 ));
			...
			float4 blendOpDest3748 = ( _DetailAlbedoPower * tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap ) );
			...
			float temp_output_3653_0 = saturate( ( ( WorldNormalVector( i , lerpResult3647 ).y * _Snow_Amount ) * ( ( _Snow_Amount * _SnowHardness ) * temp_output_3645_0 ) ) );
			float4 lerpResult3317 = lerp( lerpResult3749 , ( triplanar3738 * _SnowColor ) , temp_output_3653_0);
			...
			float lerpResult3332 = lerp( ( break3366.x * _MetallicPower ) , ( break3447.x * _SnowMetallicPower ) , temp_output_3653_0);
			o.Metallic = lerpResult3332;
			float lerpResult3345 = lerp( ( break3366.w * _SmoothnessPower ) , ( break3447.w * _SnowSmoothnessPower ) , temp_output_3653_0);
			o.Smoothness = lerpResult3345;
			float clampResult3614 = clamp( tex2D( _ShapeAmbientOcclusionG, uv_ShapeBumpMap ).g , ( 1.0 - _ShapeAmbientOcclusionPower ) , 1.0 );
