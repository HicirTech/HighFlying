Shader "NatureManufacture Shaders/Terrain/Terrain Snow Cover"
{
	Properties
	{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float4 temp_cast_0 = (localCalculateTangents203).xxxx;
				...
				float4 staticSwitch296 = float4(0,0,0,0);
			#else
				float4 staticSwitch296 = temp_cast_0;
			#endif
			v.vertex.xyz += staticSwitch296.xyz;
		}
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult31 = lerp( float3( 0,0,0 ) , UnpackScaleNormal( tex2D( _Normal0, uv_Splat0 ) ,_Normal1Power ) , clampResult23);
			float2 uv_Splat1 = i.uv_texcoord * _Splat1_ST.xy + _Splat1_ST.zw;
			float3 lerpResult32 = lerp( lerpResult31 , UnpackScaleNormal( tex2D( _Normal1, uv_Splat1 ) ,_Normal2Power ) , Splat_1_G7);
			float2 uv_Splat2 = i.uv_texcoord * _Splat2_ST.xy + _Splat2_ST.zw;
			half Splat_1_B8 = tex2DNode4.b;
			float3 lerpResult33 = lerp( lerpResult32 , UnpackScaleNormal( tex2D( _Normal2, uv_Splat2 ) ,_Normal3Power ) , Splat_1_B8);
			float2 uv_Splat3 = i.uv_texcoord * _Splat3_ST.xy + _Splat3_ST.zw;
			half Splat_1_A9 = tex2DNode4.a;
			float3 lerpResult34 = lerp( lerpResult33 , UnpackScaleNormal( tex2D( _Normal3, uv_Splat3 ) ,_Normal4Power ) , Splat_1_A9);
			...
			float4 appendResult272 = (float4(1.0 , 1.0 , 1.0 , _Smoothness0));
			float4 lerpResult24 = lerp( float4( 0,0,0,0 ) , ( appendResult272 * tex2D( _Splat0, uv_Splat0 ) ) , clampResult23);
			float4 appendResult276 = (float4(1.0 , 1.0 , 1.0 , _Smoothness1));
			float4 lerpResult25 = lerp( lerpResult24 , ( appendResult276 * tex2D( _Splat1, uv_Splat1 ) ) , Splat_1_G7);
			float4 appendResult280 = (float4(1.0 , 1.0 , 1.0 , _Smoothness2));
			float4 lerpResult26 = lerp( lerpResult25 , ( appendResult280 * tex2D( _Splat2, uv_Splat2 ) ) , Splat_1_B8);
			float4 appendResult284 = (float4(1.0 , 1.0 , 1.0 , _Smoothness3));
			float4 lerpResult27 = lerp( lerpResult26 , ( appendResult284 * tex2D( _Splat3, uv_Splat3 ) ) , Splat_1_A9);
			...
			float3 appendResult290 = (float3(lerpResult27.xyz));
			float4 clampResult288 = clamp( tex2DNode78 , float4( 0,0,0,0 ) , float4( 0.5,0.5,0.5,0 ) );
			...
			float lerpResult190 = lerp( lerpResult27.w , _SnowSmoothness , temp_output_265_0);
